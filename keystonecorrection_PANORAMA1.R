...
imgd=readTIFF("distorted1.tif")
xu=c(3975, 4131, 5449, 5910)  # top-left, bottom-left, bottom-right, top-right
yu=c(1117, 2686, 2282, 536)

# Undistorted points (destination)
# imgu=readTIFF("undistorted.tif")  # not used
xd=c(343, 363, 1727, 2286)  # top-left, bottom-left, bottom-right, top-right
yd=c(1195, 2831, 2503, 884)
...
for (x in 1:DIMXc) {
    for (y in 1:DIMYc) {
        xuyu=round(undo.keystone(x-EDGE*1.8, y-EDGE/2, k))  # add left
        if (xuyu[1]>=1 & xuyu[1]<=DIMXd & xuyu[2]>=1 & xuyu[2]<=DIMYd)
            imgc[y, x,]=imgd[xuyu[2], xuyu[1],]  # nearest neighbour interp
    }
}
writeTIFF(imgc, "corrected1.tif", bits.per.sample=16)
