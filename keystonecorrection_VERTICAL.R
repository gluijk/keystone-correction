...
plot(c(xd, xd[1]), c(yd, yd[1]), type='l', col='red', asp=1,
     xlab='X', ylab='Y', xlim=c(0, 6000), ylim=c(4000, 0))
...
for (x in 1:DIMXc) {
    for (y in 1:DIMYc) {
        xuyu=round(undo.keystone(x, y-EDGE, k))  # add top
