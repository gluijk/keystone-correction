...
for (imgnum in 1:2) {
    ...
    imgd=readTIFF(paste0("distorted", imgnum, ".tif"))
    if (imgnum==1) {
        xu=c(179, 184, 2786, 2782)  # top-left, bottom-left, bottom-right, top-right
        yu=c(121, 3862, 3870, 111)
    } else {
        xu=c(151, 637, 2532, 2443)  # top-left, bottom-left, bottom-right, top-right
        yu=c(282, 3903, 3210, 130)
    }
    ...
    # Real aspect ratio of poster:
    width=67  # picture dimensions in cm
    height=96.6
    
    f=38  # scaling factor
    xd=c(posx, posx,          posx+width*f,  posx+width*f)
    yd=c(posy, posy+height*f, posy+height*f, posy)
    ...
    # Plot trapezoids
    png(paste0("correction", imgnum, ".png"), width=512, height=700)
    ...
    writeTIFF(imgc, paste0("corrected", imgnum, ".tif"), bits.per.sample=16)
}
