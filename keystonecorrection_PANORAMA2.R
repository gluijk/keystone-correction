...
imgd=readTIFF("distorted2.tif")
xu=c(599, 543, 1783, 1812)  # top-left, bottom-left, bottom-right, top-right
yu=c(2279, 2745, 2539, 2090)

# Undistorted points (destination)
# imgu=readTIFF("undistorted.tif")  # not used
xd=c(4490, 4532, 5724, 5658)  # top-left, bottom-left, bottom-right, top-right
yd=c(2135, 2591, 2167, 1706)
...
EDGE=4000  # additional edge (Y axis)
imgc=array(0, dim=c(DIMYd+EDGE, DIMXd+EDGE, 3))  # imgc=imgd*0

DIMXc=ncol(imgc)
DIMYc=nrow(imgc)
for (x in 1:DIMXc) {
    for (y in 1:DIMYc) {
        xuyu=round(undo.keystone(x+EDGE*0.6, y-EDGE, k))  # add left
        if (xuyu[1]>=1 & xuyu[1]<=DIMXd & xuyu[2]>=1 & xuyu[2]<=DIMYd)
            imgc[y, x,]=imgd[xuyu[2], xuyu[1],]  # nearest neighbour interp
    }
}

writeTIFF(imgc, "corrected2.tif", bits.per.sample=16)
